...
class BaseCheckpointManager:
    ...
    def local_mkdir(path):
        ...
        # Use the hash of the path as the lock filename to avoid excessively long filenames
        lock_filename = f"ckpt_{hash(path) & 0xFFFFFFFF:08x}.lock"
        lock_path = os.path.join(tempfile.gettempdir(), lock_filename)
        
        try:
            with FileLock(lock_path, timeout=60):  # Set timeout for lock acquisition
                # make a new dir
                os.makedirs(path, exist_ok=True)
        except Exception as e:
            print(f"Warning: Failed to acquire lock for {path}: {e}")
            # Fallback: attempt to create directory even if lock acquisition fails
            os.makedirs(path, exist_ok=True)
