...
def compute_reward_metrics(batch):
    ...
    reward_metrics = {}
    reward_metrics["reward/mean"] = torch.mean(reward_tensor).item()
    
    # 答案正确且格式正确：score == 3
    all_correct = (reward_tensor == 3.0).float().mean()
    reward_metrics["reward/all_correct_ratio"] = all_correct.item()

    # 格式正确但答案错误：score == -0.5
    answer_error = (reward_tensor == -0.5).float().mean()
    reward_metrics["reward/answer_error_ratio"] = answer_error.item()    

    # 其他情况（格式错误或答案无法解析）
    no_answer = ((reward_tensor != 3.0) & (reward_tensor != -0.5)).float().mean()
    reward_metrics["reward/no_answer_ratio"] = no_answer.item()
